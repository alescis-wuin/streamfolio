...
const state = {
  token: localStorage.getItem('streamfolio.token'),
  user: null,
  playerCleanup: null,
...
async function route() {
  ...
  try {
    if (info.parts[0] === 'catalog') {
      await renderCatalog(info.params);
      return;
    }
    if (info.parts[0] === 'my-list') {
      await renderMyList();
      return;
    }
    if (info.parts[0] === 'title' && info.parts[1]) {
      await renderDetail(info.parts[1]);
      return;
    }
    if (info.parts[0] === 'watch' && info.parts[1]) {
      await renderPlayer(Number(info.parts[1]));
      return;
    }
    await renderHome();
  } catch (error) {
...
function renderShell(content, options = {}) {
  ...
  app.innerHTML = `
    <div class="app-shell">
      <header class="topbar">
        <a class="brand" href="#/" aria-label="Retour au catalogue Streamfolio">
        ...
        <div class="nav-actions">
          <button class="profile-button" type="button" data-logout title="Se déconnecter" aria-label="Se déconnecter du profil ${escapeHtml(state.user?.displayName || 'utilisateur')}">
            ${icons.profile}
          </button>
        </div>
      </header>
...
async function renderHome() {
  if (!state.sectionsCache || !state.genresCache) {
...
function buildRailSections(sourceSections, filters = {}, options = {}) {
  ...
  if (normalizedQuery || normalizedGenre || watchlistOnly) {
    ...
    if (watchlistOnly) {
      return all.length ? [{ id: 'watchlist-page', title: 'Ma liste', description: '', items: all }] : [];
    }
    if (normalizedQuery || normalizedGenre) {
      return all.length ? [{ id: 'filtered-catalog', title: normalizedQuery ? 'Résultats' : 'Explorer', description: '', items: all }] : [];
    }
  }

  if (requestedType === 'MOVIE') {
    return prioritizeSections(filtered, ['continue', 'watchlist', 'new', 'movies']);
  }
  if (requestedType === 'SERIES') {
    return prioritizeSections(filtered, ['continue', 'watchlist', 'new', 'series']);
  }
  return filtered;
}
    ...
    <section class="hero" aria-labelledby="hero-title">
      ...
      <div class="hero-content">
        ...
        <div class="actions">
          ...
          <button class="btn ghost" type="button" data-watchlist="${item.id}" data-active="${item.inWatchlist}">
            ${item.inWatchlist ? icons.check + ' Dans ma liste' : icons.plus + ' Ma liste'}
          </button>
        </div>
      </div>
    </section>
  ...
  return `
    <section class="discovery-strip" aria-labelledby="discovery-title">
      <div class="discovery-panel">
        <div>
          <h2 id="discovery-title">Explorer</h2>
        </div>
        <div class="genre-scroll">
          ...
          ${genres.map((genre) => {
            ...
            return `<a class="filter-chip${active ? ' active' : ''}" href="${href}"${active ? ' aria-current="page"' : ''}>${escapeHtml(genre)}</a>`;
          }).join('')}
        </div>
      </div>
    </section>
  ...
  return `
    <section class="content-section" aria-labelledby="section-${escapeHtml(section.id)}">
      <div class="section-header">
        <div>
          <h2 id="section-${escapeHtml(section.id)}">${escapeHtml(section.title)}</h2>
      ...
      <div class="carousel-shell" data-carousel-shell>
        <button class="carousel-btn carousel-btn-left" type="button" data-scroll-target="${rowId}" data-scroll-dir="-1" aria-label="Revenir dans le carrousel ${escapeHtml(section.title)}" hidden>
          <span class="carousel-triangle carousel-triangle-left" aria-hidden="true"></span>
        </button>
        <div class="card-row" data-row-id="${rowId}">
          ${section.items.map(cardView).join('')}
        </div>
        <button class="carousel-btn carousel-btn-right" type="button" data-scroll-target="${rowId}" data-scroll-dir="1" aria-label="Avancer dans le carrousel ${escapeHtml(section.title)}" hidden>
          <span class="carousel-triangle carousel-triangle-right" aria-hidden="true"></span>
        </button>
      </div>
    </section>
  `;
...
function cardView(item) {
  ...
  const previewKey = String(item.id);
  state.previewItems.set(previewKey, item);

  return `
    <article class="card">
      <div class="card-surface">
        <button class="card-thumb" type="button" data-preview-card="${previewKey}" aria-label="Afficher l’aperçu de ${escapeHtml(item.title)}">
        ...
        <button class="poster-play" type="button" data-play="${item.nextVideoId}" aria-label="Lire ${escapeHtml(item.title)}">
...
async function renderCatalog(params) {
  ...
  renderRailPage({
    data,
    genres,
    filters: { query, type, genre },
    fallbackEmpty: 'Aucun titre ne correspond à ces critères.',
    fullBleed: true,
  });
}
...
async function renderMyList() {
  ...
  renderRailPage({
    ...
    genres,
    watchlistOnly: true,
    fallbackEmpty: 'Utilise le bouton “Ma liste” sur une fiche ou une carte pour ajouter un titre.',
    fullBleed: true,
  });
}
...
async function renderDetail(slug) {
  ...
  const episodeSection = item.type === 'SERIES' ? `
      <section class="episode-list detail-glass" aria-labelledby="episodes-title">
        ...
        ${item.videos.map((video) => episodeView(video)).join('')}
      </section>` : '';
  renderShell(`
    <section class="detail-page" aria-labelledby="detail-title">
      ...
      <div class="detail-inner detail-glass">
        <aside class="detail-poster">
          <img src="${escapeHtml(item.posterPath)}" alt="Affiche de ${escapeHtml(item.title)}">
        </aside>
        <div class="detail-content">
          ...
          <div class="actions">
            ...
            <button class="btn ghost" type="button" data-watchlist="${item.id}" data-active="${item.inWatchlist}" aria-label="Basculer Ma liste">
              ${item.inWatchlist ? icons.check + ' Ma liste' : icons.plus + ' Ma liste'}
            </button>
          </div>
        </div>
      </div>
  ...
  return `
    <article class="episode">
      ...
      <div>
        ...
        <p class="muted">${formatDuration(video.durationSeconds)} · progression ${progress.toFixed(0)}%</p>
        ${progressBar(progress, `Progression ${progress.toFixed(0)}%`)}
      </div>
  ...
  renderShell(`
    <section class="player-page" aria-labelledby="player-title">
      <div class="player-meta">
        <div>
          <p class="eyebrow">${escapeHtml(playback.label)}</p>
          <h1 id="player-title">${escapeHtml(playback.videoTitle)}</h1>
          <p class="muted">${escapeHtml(playback.title)} · ${labelType(playback.type)} · ${playback.releaseYear} · ${escapeHtml(playback.maturityRating)}</p>
        </div>
        <a class="btn ghost" href="#/title/${escapeHtml(playback.titleSlug)}">Retour à la fiche</a>
      </div>
      <div class="video-frame">
        <video id="video-player" controls playsinline preload="metadata" crossorigin="anonymous">
      ...
      <div class="player-meta">
        <p class="muted">Progression sauvegardée automatiquement. Sous-titres disponibles depuis les contrôles du lecteur.</p>
        <div class="kbd-list" aria-label="Raccourcis clavier">
          <span><kbd>k</kbd> lecture/pause</span>
          <span><kbd>←</kbd><kbd>→</kbd> ±10 s</span>
...
function setupPlayer(playback) {
  ...
  video.addEventListener('loadedmetadata', () => {
    if (startPosition > 0 && startPosition < video.duration - 3) {
...
function closePreviewModal(animated = true) {
  const modal = document.querySelector('[data-preview-modal]');
  if (state.previewKeyHandler) {
    document.removeEventListener('keydown', state.previewKeyHandler);
    state.previewKeyHandler = null;
  }
...
function previewModalView(item) {
  const progress = Math.max(0, Math.min(100, item.progress?.percent || 0));
  const detailsUrl = `#/title/${escapeHtml(item.slug)}`;
  const titleId = `preview-title-${escapeHtml(item.id)}`;
  const playAction = item.nextVideoId
    ? `<button class="btn primary" type="button" data-play="${item.nextVideoId}">${icons.play} ${progress > 0 ? 'Reprendre' : 'Lire'}</button>`
    : '';

  return `
    <div class="preview-modal-backdrop" data-preview-modal>
      <section class="preview-dialog" role="dialog" aria-modal="true" aria-labelledby="${titleId}">
        <button class="preview-close" type="button" data-preview-close aria-label="Fermer l’aperçu">×</button>
        <div class="preview-dialog-media">
          <img src="${escapeHtml(item.backdropPath || item.posterPath)}" alt="" aria-hidden="true" loading="eager">
          ${item.nextVideoId ? `<button class="poster-play preview-modal-play" type="button" data-play="${item.nextVideoId}" aria-label="Lire ${escapeHtml(item.title)}"><span class="play-triangle" aria-hidden="true"></span></button>` : ''}
          <div class="preview-dialog-title-band"><h2 id="${titleId}">${escapeHtml(item.title)}</h2></div>
        </div>
        <div class="preview-dialog-body">
          <div class="preview-dialog-topline">
            <div class="preview-meta preview-modal-meta">
              <strong>${labelType(item.type)}</strong>
              <span>${escapeHtml(contentLengthLabel(item))}</span>
              <span>${item.releaseYear}</span>
              <span>${escapeHtml(item.maturityRating)}</span>
            </div>
            <a class="details-orb details-orb-modal" href="${detailsUrl}" data-preview-detail aria-label="Afficher la fiche complète de ${escapeHtml(item.title)}">
              <span aria-hidden="true"></span>
            </a>
          </div>
          <p class="preview-modal-description">${escapeHtml(item.synopsis || item.tagline || '')}</p>
          <div class="preview-genres preview-modal-genres">${(item.genres || []).map((genre) => `<span>${escapeHtml(genre)}</span>`).join('')}</div>
          ${progress > 2 ? progressBar(progress, `Progression ${progress.toFixed(0)}%`) : ''}
...
async function handleSubmit(event) {
  ...
  if (loginForm) {
    ...
    try {
      ...
      localStorage.setItem('streamfolio.token', response.token);
      location.hash = '#/';
      route();
    } catch (error) {
...
async function handleClick(event) {
  if (event.target.matches('[data-preview-modal]')) {
    closePreviewModal();
    return;
  }
  ...
  const previewDetail = event.target.closest('[data-preview-detail]');
  if (previewDetail) {
    closePreviewModal(false);
    return;
  }
  ...
  if (previewCard) {
    event.preventDefault();
    const item = state.previewItems.get(previewCard.getAttribute('data-preview-card'));
...
function logout(refresh = true) {
  state.token = null;
  state.user = null;
  localStorage.removeItem('streamfolio.token');
  if (refresh) {
    location.hash = '#/';
    route();
  }
...
function metaRow(item) {
  return `
    <div class="meta-row" aria-label="Métadonnées">
      <strong>${labelType(item.type)}</strong>
      <span aria-hidden="true">•</span>
      <span>${item.releaseYear}</span>
      <span aria-hidden="true">•</span>
...
function filterChip(label, href, active) {
  return `<a class="filter-chip${active ? ' active' : ''}" href="${escapeHtml(href)}"${active ? ' aria-current="page"' : ''}>${escapeHtml(label)}</a>`;
}

function catalogHash(currentParams, changes) {
...
function scrollCarousel(row, direction) {
  ...
  if (direction < 0 && row.scrollLeft <= threshold) {
